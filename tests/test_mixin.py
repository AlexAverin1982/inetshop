...
def test_mixin_output() -> None:
    ...
    expected_output = "Smartphone('smart', 'smart_desc', '1000.0', '100', '999.9', 'iphone', '256', 'black')"
    assert expected_output == output[-1]

    src.classes.LawnGrass(
        "greengrass", "grass_desc", 500.0, 10, "Russia", "1 year", "light_green"
    )
    expected_output = "LawnGrass('greengrass', 'grass_desc', '500.0', '10', 'Russia', '1 year', 'light_green')"
    assert expected_output == output[-1]
